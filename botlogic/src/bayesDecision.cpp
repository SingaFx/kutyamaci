...
double manipulateEQRaise(double EQ, double betsize, CurrentGameInfo& game)
{
	...
	if (isFish(game) && game.getOpponentsInfo().size() == 1) EQ = modifyValue(EQ, 0.02);

	if (game.getStreet() == 0)
	{
		if (betsize < 18 && heroInPosition(game)) EQ = modifyValue(EQ, 0.04);
		...
		if (isStrongPlayer(game) && betsize > 30 && maxStackSize >= 100 && !(game.getHand().isPocket() && game.getHand().getCard1().getRank() == 'A'))
		{
			logger.logExp("\t\t\tModifying Raise: if NOT AA EQ -30%", BOT_LOGIC);
			EQ = modifyValue(EQ, -0.40);
		}
		...
		if (betsize > 2 && (!game.getHand().isOOP3Bet() && !game.getHand().isAxs() && !game.getHand().isStrongBroadway() && !game.getHand().isSuitedBroadway()))
		{
			EQ = modifyValue(EQ, -0.20);
			logger.logExp("\t\t\tModifying Raise: 3BET WEAK HAND EQ -20%", BOT_LOGIC);
		}

		if (betsize > 15 && maxStackSize >= 95 && !game.getHand().is100BBStackOff())
		{
			EQ = modifyValue(EQ, -0.30);
			logger.logExp("\t\t\tModifying Raise: STACKOFF 100BB WEAK HAND EQ -20%", BOT_LOGIC);
		}

		if (betsize >= 20 && isStrongPlayer(game) && !isRegFish(game) && maxStackSize >= 95 && !game.getHand().isKingsPlus())
		{
			EQ = modifyValue(EQ, -0.30);
		}

		if (betsize >= 10 && isStrongPlayer(game) && !isRegFish(game) && maxStackSize >= 95 && !game.getHand().is100BBStackOff())
		{
			EQ = modifyValue(EQ, -0.2);
		}

		if (betsize > 30 && maxStackSize >= 95 && !game.getHand().isKingsPlus())
		{
			EQ = modifyValue(EQ, -0.20);
			logger.logExp("\t\t\tModifying Raise: STACKOFF 100BB WEAK HAND EQ -20%", BOT_LOGIC);
		}

		if (isTightRegular(game) && !game.getHand().is100BBStackOff()) EQ = modifyValue(EQ, -0.2);
	...
	else if (game.getStreet() == 1)
	{
		if (!heroInPosition(game)) 
		{
			...
			logger.logExp("\t\t\tModifying Raise: not in position -3%", BOT_LOGIC);
	...
	{	
		...
		if (heroInPosition(game))
		{
			if (!isFish(game)) EQ = modifyValue(EQ, -0.05);
		}
		
		if (str == 4) EQ = modifyValue(EQ, -0.2);
		if (currentbet >= 2 && maxStackSize >= 90)
		{
			logger.logExp("\t\t\tModifying Raise: RAISE EQ -5%", BOT_LOGIC);
			EQ = modifyValue(EQ, -0.05);
			if (str >= 2) EQ = modifyValue(EQ, -0.1);
		}
	}
	else if (game.getStreet() == 3)
	{
		bool is3StreetBet = false;
		if (game.getFlopPotSize() < 20 && game.getPotcommon() >= 40) is3StreetBet = true;
		if (isStrongPlayer(game) && !isRegFish(game) && is3StreetBet) EQ = modifyValue(EQ, -0.05);
		if (currentbet >= 2 && maxStackSize >= 100)
		{
			logger.logExp("\t\t\tModifying Raise: RAISE EQ -5%", BOT_LOGIC);
			EQ = modifyValue(EQ, -0.05);
...
double manipulateEQRaiseAllIn(double EQ, double betsize, CurrentGameInfo& game)
{
	...
	logger.logExp("\t\t\tModifying AI: originalEffectiv: ", originalEffectiv, BOT_LOGIC);
	logger.logExp("\t\t\tModifying AI: effectiv: ", effectiv, BOT_LOGIC);

	if (game.getStreet() > 0 && ((game.getPotcommon() < 20 && game.getFlopPotSize() < 20) || game.getOpponentsInfo().size() > 2) && game.getBiggestBet() < game.getPotcommon() / 5 && effectiv > game.getPotcommon() * 2 && originalEffectiv > 80)
	{
		EQ = modifyValue(EQ, -0.05);
		logger.logExp("\t\t\tModifying AI: normal pot -5%", BOT_LOGIC);
		if (originalEffectiv > 100)
		{
			logger.logExp("\t\t\tModifying AI: normal pot full stack -5%", BOT_LOGIC);
			EQ = modifyValue(EQ, -0.05);
		}
		
	} 
	else if (game.getStreet() > 0 && ((game.getPotcommon() < 20 && game.getFlopPotSize() < 20) || game.getOpponentsInfo().size() > 2)  && effectiv > game.getPotcommon() * 2 && originalEffectiv > 80)
	{
		EQ = modifyValue(EQ, -0.05);
		logger.logExp("\t\t\tModifying AI: normal pot -5%", BOT_LOGIC);
	}

	if (game.getFlopPotSize() < 5)
	{
		if (isFish(game)) EQ = modifyValue(EQ, -0.05);
		else
		{
	...
	if (game.getStreet() > 0 && isFullStackRegular(game) && isStrongPlayer(game) && !isRegFish(game)) 
	{
		
		if (game.getFlopPotSize() > 20 && originalEffectiv < 200)
		{
			logger.logExp("\t\t\tModifying AI: REG in play 3bet pot -2%", BOT_LOGIC);
			EQ = modifyValue(EQ, -0.02);
		}
		else
		{
			logger.logExp("\t\t\tModifying AI: REG in play normal pot -9%", BOT_LOGIC);
			EQ = modifyValue(EQ, -0.09);
			//if (game.getStreet() == 3) EQ = modifyValue(EQ, 0.02);
		}
	}
	else if (isFullStackRegular(game))
	{
		if (game.getFlopPotSize() > 20 && originalEffectiv < 200)
		{
		}
		...
		{
			logger.logExp("\t\t\tModifying AI: SEMI REG in play normal pot -3%", BOT_LOGIC);
			EQ = modifyValue(EQ, -0.03);
		}
	}

	if (isTightRegular(game) && originalEffectiv >= 90)
	{
		logger.logExp("\t\t\tModifying AI: TIGHT REG in play -3%", BOT_LOGIC);
		EQ = modifyValue(EQ, -0.03);
	}
	...
	if (game.getStreet() == 0)
	{
		...
		if (game.getBblind() > 0.08)
		{
			...
			if (betsize > 30 && originalEffectiv > 200 && !(game.getHand().isPocket() && game.getHand().getCard1().getRank() == 'A'))
			{
				EQ = modifyValue(EQ, -0.20);
				logger.logExp("\t\t\tModifying Raise: STACKOFF 100BB WEAK HAND EQ -20%", BOT_LOGIC);
			}
		...
		{
			if (isTightRegular(game) && betsize > 35 && originalEffectiv > maxStack && !(game.getHand().isPocket() && game.getHand().getCard1().getRank() == 'A'))
			{
				logger.logExp("\t\t\tModifying Raise: if NOT AA EQ -30%", BOT_LOGIC);
				EQ = modifyValue(EQ, -0.30);
			}
	...
	{
		bool is3StreetBet = false;
		if (game.getFlopPotSize() < 20 && game.getFlopPotSize() > game.getPotcommon()) is3StreetBet = true;
		if (isStrongPlayer(game) && !isRegFish(game) && is3StreetBet) EQ = modifyValue(EQ, -0.05);
		if (game.getBiggestBet() > game.getPotcommon() && isStrongPlayer(game) && !isRegFish(game)) EQ = modifyValue(EQ, -0.03);
		if (game.getFlopPotSize() < 20 && originalEffectiv > 90) EQ = modifyValue(EQ, -0.04);
		...
		if (game.getBiggestBet() < game.getPotcommon() / 5 && effectiv > 2 * game.getPotcommon())
		{
			EQ = modifyValue(EQ, -0.05);
		}
	}
...
double manipulateEQCall(double EQ, CurrentGameInfo& game)
{
	...
	{
		EQ = modifyValue(EQ, -0.05);
		logger.logExp("\t\t\tModifying Call: TIGHT REGULAR -5%", BOT_LOGIC);
	}
	...
	if (game.getStreet() == 0)
	{
		...
		if (!game.getHand().is100BBStackOff() && !game.getHand().isBigSC() && !game.getHand().isOOP3Bet() && !game.getHand().isPocket() && !game.getHand().isSuitedBroadway())
		{
			EQ = modifyValue(EQ, -0.30);
		}
	...
	{		
		...
		if (str > 1 && str < 5 && !(game.getBiggestBet() < game.getPotcommon() * 0.1))
		{
			EQ = modifyValue(EQ, -0.05);
			logger.logExp("\t\tModifying Call: str > 1 && str < 5 vsBiggerBet -5%", BOT_LOGIC);
		}

		if (heroInPosition(game) && abs(game.getAmountToCall() - 0) < 0.001 && game.getPotcommon() >= 20 && str > 1)
		{
			EQ = modifyValue(EQ, 0.1);
			logger.logExp("\t\tBIGPOT CHBH EQ +10%", BOT_LOGIC);
		} 
	}
	else if (game.getStreet() == 2)
	{
		...
		if (game.getAmountToCall() > 30 && game.getPotcommon() < 4 * game.getAmountToCall()) EQ = modifyValue(EQ, -0.08);
	...
	else if (game.getStreet() == 3)
	{
		...
		EQ = modifyValue(EQ, -0.02);
		logger.logExp("\t\tModifying Call: DEFAULT -2%", BOT_LOGIC);
		if (isFullStackRegular(game)) 
		{
			EQ = modifyValue(EQ, -0.04);
			logger.logExp("\t\tModifying Call: fullStackRegular -5%", BOT_LOGIC);
		}
		...
		bool is3StreetBet = false;
		if (game.getFlopPotSize() < 20 && game.getFlopPotSize() < game.getPotcommon() && game.getPotcommon() >= 40) is3StreetBet = true;
		if (isStrongPlayer(game) && !isRegFish(game) && is3StreetBet) EQ = modifyValue(EQ, -0.07);
		...
		if (game.getPotcommon() > 3 * game.getBiggestBet() && game.getPotcommon() < 100) EQ = modifyValue(EQ, 0.03);
	}
...
double manipulateFE(double FE, double betsize, CurrentGameInfo& game, CurrentPlayerInfo& player)
{
	...
	else if (game.getStreet() == 1)
	{
		...
		if (game.getOpponentsInfo().size() > 1) FE = modifyValue(FE, 0.02);
		if (game.getBiggestBet() > 1) FE = modifyValue(FE, -0.1);
		logger.logExp("\t\t\tModifying FE: DEFAULT -15%", BOT_LOGIC);

		if (game.getPotcommon() > 28 && !heroInPosition(game))
		{
	...
	else if (game.getStreet() == 2)
	{
		...
		logger.logExp("\t\t\tModifying FE: DEFAULT -20", BOT_LOGIC);
...
double BayesDecision::calculateEQ(vector<PlayerRange>& ranges, vector<Card>& board, Hand& hand, CurrentGameInfo& game)
{
	...
	if (isStrongPlayer(game) && !isRegFish(game) && !isFish(game)) eq = modifyValue(eq, -0.03);
...
double BayesDecision::calculateEVRaise(CurrentGameInfo& gameInfo, vector<PlayerRange>& callRaiseRanges, vector<double>& foldEquities, double betsize, int& moveType, double callEQ, bool allIn = false)
{ 
	Logger& logger = Logger::getLogger(BOT_LOGIC);
...
Action BayesDecision::makeDecision(CurrentGameInfo& game, vector<PlayerRange>& ranges, BayesUserPreflop& preflop, BayesUserFlop& flop, BayesUserTurn& turn, BayesUserRiver& river)
{
	...
	if (committed(game) && game.getStreet() != 3)
	{
		...
		int moveType;
		double EVAI = calculateEVRaise(game, aktRanges, foldEquities, betsize, moveType, 100, true);
	...
	if (game.getStreet() == 0)
	{
		...
		for (int i = ranges.size() > 2 ? betSizes.size() - 1 : 0; i < betSizes.size(); ++i)
		{
			...
			int moveType;
			double evraise = calculateEVRaise(game, aktRanges, foldEquities, betsize, moveType, callEQ, allIn);
	...
	if (game.getStreet() == 1)
	{
		...
		if (!Evaluator::flopTotalAir(game.getHand(), game.getBoard()))
		{
			for (int i = ranges.size() > 2 ? betSizes.size() - 1 : 0; i < betSizes.size(); ++i)
			{
		...
		//IP SZARAZ BOARDON EROS HAND
		int boardType = Evaluator::boardType(game.getBoard());
		if (ranges.size() == 1 && EVRAISE > 0 && boardType < 1 && maxRaiseType == 0 && heroInPosition(game) && game.getBiggestBet() * game.getBblind() <= potcommon 
		...
		if (ranges.size() == 1 && isFullStackRegular(game) && !heroInPosition(game) && game.getHand().isPocket() && !game.getHand().getCard1().isBroadway() && boardType < 2 && (str == 0 || str == 3) &&
		...
		//PLAY VS AGGRO CHBH
		if (ranges.size() == 1 && heroInPosition(game) && game.getBiggestBet() * game.getBblind() < potcommon * 0.2 && maxRaiseType == 0 && boardType < 2 && isFullStackRegular(game))
		{
			int random;
			random = random % 3;
			if ((str != 1 && str != 2) || str == 0 || str == 7 || canCallAfterRaise(game, ranges[0], maxRaiseSize, preflop, flop, turn, river)) random = 1;//nem chbh
	...
	if (game.getStreet() == 2)
	{
		...
		//PLAY VS AGGRO CHBH
		if (ranges.size() == 1 && heroInPosition(game) && game.getBiggestBet() * game.getBblind() < potcommon * 0.2 && maxRaiseType < 2 && boardType < 2 && isStrongPlayer(game) && abs(game.getPotcommon() - game.getFlopPotSize()) > 0.01)
		{
			int random;	
			random = rand() % 2;

			if ((str != 1 && str != 2) || str == 0 || str == 7 || canCallAfterRaise(game, ranges[0], maxRaiseSize, preflop, flop, turn, river)) random = 1; 
	...
	if (game.getStreet() == 3)
	{
		...
		if (EVRAISE == -100000 && EVCALL < 0) 
		{
			logger.logExp("The EVRAISE is -10000. No action is taken.", BOT_LOGIC);
			return Action('n', 0);
		}
