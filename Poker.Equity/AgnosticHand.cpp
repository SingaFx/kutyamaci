...
int AgnosticHand::Instantiate(const char* handText, const char* deadText, vector<StdDeck_CardMask>& specificHands)
{
	...
	if (deadText && strlen(deadText))
	{
		...
		{
			rank = KCard::CharToRank(*pCard);
			suit = KCard::CharToSuit(*(pCard+1));
...
int AgnosticHand::Instantiate(const char* handText, StdDeck_CardMask deadCards, vector<StdDeck_CardMask>& specificHands)
{
	...
	if (isSlice)
	{
		...
		handRanks[0] = KCard::CharToRank(handFloor[0]);
		handRanks[1] = KCard::CharToRank(handFloor[1]);
		rankCeils[0] = KCard::CharToRank(handCeil[0]);
		rankCeils[1] = KCard::CharToRank(handCeil[1]);
	}
	else
	{
		handRanks[0] = KCard::CharToRank(handText[0]);
		handRanks[1] = KCard::CharToRank(handText[1]);
		rankCeils[0] = isPlus ? KCard::Ace : handRanks[0];
		rankCeils[1] = KCard::King;
	}
	...
	else if (IsSuited(handText))
	{
		...
		if ((isPlus || isSlice) && (handRanks[0] == KCard::Ace))
			rank0Increment = 0;
	...
	else
	{
		...
		int maxRank = isPlus ? KCard::Ace : handRanks[0];
